...
var $author$project$Picshare$errorMessage = function (error) {
	if (error.$ === 'BadBody') {
		return 'アプリケーションがJSONデータを処理できませんでした。\n            現在この問題に対処しています。';
	} else {
		return 'アプリケーションがJSONデータを処理できませんでした。\n            後ほど再度お試しください。';
	}
};
					...
					[
						...
						_List_fromArray(
							[
								...
								$elm$html$Html$Attributes$placeholder('コメントを追加...'),
...
var $author$project$Picshare$viewStreamNotification = function (queue) {
	if (!queue.b) {
		...
		var content = '新しい写真を見る: ' + ($elm$core$String$fromInt(
			$elm$core$List$length(queue)) + '件');
		return A2(
			$elm$html$Html$div,
			_List_fromArray(
				[
